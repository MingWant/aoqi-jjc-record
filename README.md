...
历史补录页默认打开“时间矩阵”模式：行是已有玩家，列是竞技场届次内所有结算周，勾选表示该周进入前五席位。可以从玩家档案目录添加行、批量勾选多个周次后一次保存；未填满战皇席位的周次会保存为 `partial`。玩家档案页也可以先手动创建全新玩家身份，再从矩阵或详细周榜中补录。需要精确填写战区和名次时，可切换到“详细周榜”模式。

“名人堂候选”页也提供只读时间矩阵：可在“本届 / 三届周期”窗口之间切换，并查看每位候选人在各结算周的战皇记录；当前并列候选会用浅金色行标记。需要按战皇数和连皇数查看时可切换回“候选排行”。

## 竞技场届次与名人堂

首次启动会按 `.env` 创建初始竞技场届次。经典竞技场和传奇竞技场共用同一届次，名人堂只汇总竞技场届次数据，不单独维护届次。`ARENA_SEASON_END` 可直接指定结束时间；也可以填写 `ARENA_SEASON_WEEKS`，系统会从开始时间起经过完整周数，并保持结束时间与开始时间为同一星期、同一时刻。网站“系统”页可以新增或编辑竞技场届次，填写计划周数时周数优先于手动结束时间；不填写周数和结束时间则为开放式届次。
...
当前 H5 服务代码从第 29 届起按固定三届周期合并，即第 29–31 届、第 32–34 届，之后依次类推，并非滚动“最近三届”。网站的“三届周期”只累计所选竞技场届次所在周期截至该届已有的数据；例如选择第 32 届时只计入第 32 届。所有周榜均保存原始依据，官方口径变化时可以重新计算。

如果某个已到期的结算周完全没有记录，会作为缺失周插入时间轴并打断连皇；`partial` 周仍会统计已经明确录入的战皇席位，但页面会单独显示不完整周数。

## 管理安全

网站和统计 API 默认需要访问密钥；立即采集、手动固化、导入快照和竞技场届次修改还需要管理密码。两把密钥可以设置成同一个随机值，也可以分开给普通查看者和管理员使用。
...
- `POST /api/admin/seasons`：新增或编辑竞技场届次；传入 `weeks` 可按计划周数自动计算 `endsAt`，传入 `endsAt` 可指定自定义周期。
