...
首次启动会按 `.env` 创建初始竞技场届次。经典竞技场和传奇竞技场共用同一届次，名人堂只汇总竞技场届次数据，不单独维护届次。`ARENA_SEASON_END` 可直接指定结束时间；也可以填写 `ARENA_SEASON_WEEKS`，系统按最后一个竞技场结算截止时间自动计算结束时间。网站“系统”页可以新增或编辑竞技场届次，填写计划周数时周数优先于手动结束时间；不填写周数和结束时间则为开放式届次。
